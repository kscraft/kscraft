...
if (!function_exists('reyajaxfilter_terms_output')):
	...
	function reyajaxfilter_terms_output($args) {
		...
		// if ($args['taxonomy'] === 'product_cat' && is_tax()) { // maybe this is how it should be
				...
				if ($count > 0 || $force_show) {
					...
					if( ! $args['dropdown'] ){
						...
						if( $args['alphabetic_menu'] && strlen($_parent_term_name) > 0 ){
							$li_attribute = sprintf('data-letter="%s"', substr($_parent_term_name, 0, 1) );
						}
						...
						$html .= sprintf(
							'<a href="%1$s" data-key="%2$s" data-value="%3$s" data-multiple-filter="%4$s">%7$s %5$s %6$s</a>',
...
if (!function_exists('reyajaxfilter_list_terms')):
	...
	function reyajaxfilter_list_terms($args) {
		...
		foreach ($args['terms'] as $parent_term_id) {
			...
			if ($count > 0 || $force_show) {
				...
				if( $args['alphabetic_menu'] && strlen($parent_term->name) > 0 ){
					$li_attribute = sprintf('data-letter="%s"', substr($parent_term->name, 0, 1) );
				}
				...
				$html .= wp_kses_post( apply_filters( 'woocommerce_layered_nav_term_html', $term_html, $parent_term, $link, $count ) );
		...
		$list_classes = $list_styles = [];

		if( $custom_height = absint($args['custom_height'] ) ){
			$list_styles[] = sprintf('height:%spx', $custom_height);
			$list_classes[] = '--custom-height';
		}
		...
		$list_html .= sprintf('<div class="reyajfilter-layered-nav %s">', implode(' ', $list_classes));
			$list_html .= sprintf('<ul style="%s">', implode(' ', $list_styles));
			$list_html .= $html;
			$list_html .= '</ul>';
