...
if( defined( 'ELEMENTOR_PRO_VERSION' ) && !class_exists('ReyCore_Compatibility__ElementorPro') ):
	...
	class ReyCore_Compatibility__ElementorPro
	{
		...
		public function on_elementor_pro_init() {
			...
			add_action( 'elementor/frontend/widget/before_render', [$this, 'before_render_related_products'], 10);
		...
		function loop_props_in_elements( $element ){
			...
			if( ! in_array($element->get_unique_name(), $widgets) ){
				return;
			}
		...
		/**
		 * Fix Related products cols
		 *
		 * @since 1.6.4
		 **/
		function before_render_related_products( $element )
		{

			if( $element->get_unique_name() !== 'woocommerce-product-related' ){
				return;
			}

			wc_set_loop_prop('columns', $element->get_settings_for_display('columns'));
