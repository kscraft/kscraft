		...
		<td class="order-data">
			<table>
				...
				<tr class="proforma-number">
					<th><?php _e( 'Proforma Number:', 'woocommerce-quotation' ); ?></th>
					<td><?php $this->number( 'adq_proforma' ); ?></td>
				</tr>
				<?php } ?>
				<?php if ( isset( $this->settings['display_date'] ) ) { ?>
				<tr class="invoice-date">
					<th><?php _e( 'Date:', 'woocommerce-pdf-invoices-packing-slips' ); ?></th>
					<td><?php $this->date( 'adq_proforma' ); ?></td>
				</tr> 
				<?php } ?>
...
<table class="order-details">
	<thead>
		<tr>
			<?php
			if ( class_exists( 'WPO_WCPDF_Templates' ) && $headers = wpo_wcpdf_templates_get_table_headers( $this ) ) :
	...
	<tbody>
		<?php
        if ( class_exists( 'WPO_WCPDF_Templates' ) && $tbody = wpo_wcpdf_templates_get_table_body( $this ) ) :
            if( sizeof( $tbody ) > 0 ) :
                    ...
                    <tr class="<?php echo apply_filters( 'wpo_wcpdf_item_row_class', $item_id, $this->type, $this->order, $item_id ); ?>">
                        <td class="product">
                            ...
                            <dl class="meta">
                                ...
                                <?php
                                if ( ! empty( $item['sku'] ) ) : ?>
                                    <dt class="sku"><?php _e( 'SKU:', 'woocommerce-pdf-invoices-packing-slips' ); ?></dt><dd class="sku"><?php echo $item['sku']; ?></dd>
                                <?php 
                                endif;
                                if ( ! empty( $item['weight'] ) ) : ?>
                                <dt class="weight"><?php _e( 'Weight:', 'woocommerce-pdf-invoices-packing-slips' ); ?></dt><dd class="weight"><?php echo $item['weight']; ?><?php echo get_option( 'woocommerce_weight_unit' ); ?></dd>
                                <?php 
                                endif; ?>
                            </dl>
    ...
    <?php if ( ! class_exists( 'WPO_WCPDF_Templates' ) ) : ?>
	<tfoot>
		<tr class="no-borders">
			<td class="no-borders">
