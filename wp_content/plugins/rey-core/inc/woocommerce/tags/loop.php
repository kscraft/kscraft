...
class ReyCore_WooCommerce_Loop
{
	...
	public function component_product_category()
	{
		...
		if ( $product && $id = $product->get_id() ) {
			echo wc_get_product_category_list(
				$id,
				$sep = ', ',
				$before = '<div class="rey-productCategories">',
				$after = '</div>'
			);
		}
	}
	 ...
	 * @since 1.0.0
	 */
	public function component_discount_percentage_to_price( $html )
	{
		if( ! is_product() ){
	...
	*/
	...
	public function get_loop_skins(){
		return apply_filters('reycore/woocommerce/loop_skins', $this->_skins );
	}
