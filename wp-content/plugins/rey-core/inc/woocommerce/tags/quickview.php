...
class ReyCore_WooCommerce_QuickView {
	...
	public static function get_button_html( $product_id = '', $button_class = 'button' )
	{
		...
		if( $type === 'icon' ){
			$button_class = '';
			$button_content = reycore__get_svg_icon__core(['id'=>'reycore-icon-eye']);
		}

		if( $type === '1' && ($btn_style = get_theme_mod('loop_quickview_style', 'under')) ){
			$button_class .= ' rey-btn--' . $btn_style;
		}

		$btn_html = sprintf('<button class="%s rey-quickviewBtn js-rey-quickviewBtn" data-id="%s" title="%s">%s</button>', $button_class, esc_attr($id), $text, $button_content );
