...
class ReyCore
{
	...
	private function define_constants()
	{
		...
		if( !defined('REY_DEV_MODE') ){
			$this->define( 'REY_CORE_VERSION', '1.6.5' );
		}
		else {
			// cache buster
	...
	function show_notices()
	{
		...
		if ( defined('REY_THEME_VERSION') && version_compare( REY_THEME_VERSION, REY_CORE_THEME_MIN_VERSION, '<' ) ) {
			$errors[] = sprintf(__( '<strong>Rey theme is outdated.</strong> Please check the <a href="%s">Updates</a> page and update it to its latest version.', 'rey-core' ) , esc_url( admin_url( 'update-core.php?force-check=1' ) ));
		}
