			...
			if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 ) {
				...
				<tr class="adq_list <?php echo esc_attr( apply_filters( 'woocommerce_quote_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
					...
					<td class="product-name" data-title="<?php esc_attr_e( 'Product', 'woocommerce' ); ?>">
						...
						$datacust = WC()->cart->get_item_data( $cart_item );
						$customst = explode(" ",$datacust);
						$height = preg_replace( '/[^\d]/', '', $customst[22]);
						$width = preg_replace( '/[^\d]/', '', $customst[47]);
					...
					<td class="product-subtotal" data-title="<?php esc_attr_e( 'Total', 'woocommerce' ); ?>">
                        ...
                        <?php
							$tqamount =(((($height*$width)/12)*$product_price)*$cart_item['quantity']);
							setlocale(LC_MONETARY, 'en_IN');
							echo $tqfamount = money_format('%!i', $tqamount);
								//echo (($height*$width)/12)*$product_price*$product_quantity;
                            /*if ( StaticAdqQuoteRequest::can_show_price( $_product ) ) {
