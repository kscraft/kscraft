                        ...
                        if ($.inArray(type, ["price", "value", "quantity", "count"]) !== -1) {
                            ...
                            if (elementWrap.length) {
                                ...
                                if (!noevents && ! thisElement.data("addedfieldtcEpoBeforeOptionPriceCalculation")){
                                    ...
                                    jWindow.on("tcEpoAfterOptionPriceCalculation.math", function () {
                                        elementWrap.data("fetchOptionPricesField", false);
                                        elementWrap.data("fetchOptionPricesField-fee", false);
                                    });
                                ...
                                if (elementWrap.is(".tc-container-enabled")) {
                                    ...
                                    if (element.is(".tmcp-field")) {
                                        if (elementWrap.data("fetchOptionPricesField")){
                                            val = elementWrap.data("fetchOptionPricesField");
                                        } else {
                                            val = fetchOptionPrices(epoObject, elementWrap, ".tmcp-field", 0, 0, [], true);
                                            elementWrap.data("fetchOptionPricesField", val);
                                        }                                        
                                    } else if (element.is(".tmcp-fee-field")) {
                                        if (elementWrap.data("fetchOptionPricesField-fee")){
                                            val = elementWrap.data("fetchOptionPricesField-fee");
                                        } else {
                                            val = fetchOptionPrices(epoObject, elementWrap, ".tmcp-fee-field", 0, 0, [], true);
                                            elementWrap.data("fetchOptionPricesField-fee", val);
                                        }                                        
                                    }
        ...
        jDocument.ajaxSuccess(function (event, request, settings) {
            ...
            Object.keys(qv_container).forEach(function (key) {
                ...
                if (key !== "yith_quick_view_plugin" && container.length && (container.find(".product").length > 0 || noProductCheck )) {
                    ...
                    if (key === "jet_popup_get_content" || key === "wp_food" || key === "woodmart_quick_shop" || key === "fwoodmart_quick_view" || key === "lightboxpro" || key === "jckqv_quick_view" || key === "yith_quick_view_plugin" || key === "theme_flatsome") {
