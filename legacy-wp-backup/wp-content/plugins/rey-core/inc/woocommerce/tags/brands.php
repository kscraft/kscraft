...
class ReyCore_WooCommerce_Brands
{
	...
	function saswp_structured_data_brands( $data ){
		...
		if( isset($data['brand']) ){
			$data['brand']['name'] = $this->get_brand();
		}
	...
	{
		return wc_clean( apply_filters('reycore/woocommerce/brand_attribute', 'brand') );
	}
	...
	function get_brand_link($brand = ''){
		...
		if( $this->has_archive ){
			...
			if( empty($this->brand_terms) ){
				$this->brand_terms = get_terms( wc_attribute_taxonomy_name( $brand_attribute_name ) );
			}

			$brand_names = array_values ( array_filter( $this->brand_terms, function( $item, $i ) use ( $brand ) {
				if( is_array($brand) ){
					return in_array( $item->name, $brand, true );
				}
				else {
					return isset($item->name) && $item->name === $brand;
				}
			}, ARRAY_FILTER_USE_BOTH ) );

			return esc_url( isset($brand_names[0] ) ? get_term_link( $brand_names[0] ) : '' );
		}

		$url = get_permalink( wc_get_page_id( 'shop' ) );

		// Default attribute filtering
		$brand_url = sprintf( '%s?filter_%s=%s', $url, $brand_attribute_name, $brand );
		$brand_url = apply_filters('reycore/woocommerce/brands/url', $brand_url, $brand_attribute_name, $url );

		return esc_url( $brand_url );
	}
	...
	function get_brands_html(){
		if ( $this->brands_tax_exists() && $brand = $this->get_brand() ) {
			$brand_html = sprintf( '<div class="rey-brandLink"><a href="%s">%s</a></div>', $this->get_brand_link( $brand ), $brand);
			echo apply_filters('reycore/woocommerce/brands/html', $brand_html);
		}
	}
