...
class ReyCore_WooCommerce_RelatedProducts
{
	...
	function add_extra_product_edit_options(){
		...
		<div class="options_group">
			...
			<p class="form-field hide_if_grouped hide_if_external">
				...
				<select class="wc-product-search" multiple="multiple" style="width: 50%;" id="rey_related_products" name="<?php self::META_KEY ?>[]" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'rey-core' ); ?>" data-action="woocommerce_json_search_products_and_variations" data-exclude="<?php echo intval( get_the_ID() ); ?>">
					<?php
	...
	public function process_extra_product_edit_options( $product_id ) {
		...
		if ( $this->custom_enabled() && ! empty( $_POST[ self::META_KEY ] ) && $related = wc_clean( $_POST[ self::META_KEY ] ) ) {
