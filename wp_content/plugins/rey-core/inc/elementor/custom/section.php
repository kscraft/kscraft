	...
	class ReyCore_Element_Section {
		...
		function slideshow_settings( $element )
		{
			$element->start_controls_section(
				...
				[
					'label' => __( 'Background Slideshow (Rey Theme)', 'rey-core' ),
		...
		function layout_settings( $element )
		{
			...
			$element->add_control(
				'rey_flex_wrap',
				[
					'label' => __( 'Multi-rows (Flex Wrap)', 'rey-core' ),
		...
		function tabs_settings( $element )
		{
			...
			$element->start_controls_section(
				'section_tabs',
				[
					'label' => __( 'Tabs Settings', 'rey-core' ),
					'tab' => \Elementor\Controls_Manager::TAB_LAYOUT
				]
			);
		...
		function effects_settings( $element )
		{
			...
			if( ReyCoreElementor::animations_enabled() ):
				...
				$element->add_control(
					'rey_animation_type',
					[
						'label' => __( 'Entrace Effect (Rey)', 'rey-core' ),
						'type' => \Elementor\Controls_Manager::SELECT,
						'default' => '',
						'options' => [
							''  => __( '- Select -', 'rey-core' ),
			...
			$element->add_control(
				'rey_scroll_effects',
				[
					'label' => __( 'Scroll Effects (Rey)', 'rey-core' ),
					'type' => \Elementor\Controls_Manager::SELECT,
					'default' => '',
					'options' => [
						''  => __( 'None', 'rey-core' ),
		...
		function custom_css_settings(  $element ){
			...
			$element->start_controls_section(
				...
				[
					'label' => sprintf( '<span>%s</span><span class="rey-hasStylesNotice">%s</span>', __( 'Rey - Custom CSS', 'rey-core' ), __( 'Has Styles!', 'rey-core' ) ),
			...
			$css_desc .= sprintf('<p class="rey-addMargin"><span></span><select class="js-insertSnippetToEditor">
				...
				<option value="@media (max-width:767px) {}">< 767px</option>
				<option value="@media (min-width:768px) {}">> 768px</option>
				<option value="@media (min-width:768px) and (max-width:1024px) {}">768px to 1024px</option>
				<option value="@media (min-width:1025px) {}">> 1025px</option>
				<option value="@media (min-width:1025px) and (max-width:1440px) {}">1025px to 1440px</option>
		...
		function before_render( $element )
		{
			...
			if( ReyCoreElementor::animations_enabled() && $settings['rey_animation_type'] != '' ):
				...
				if( $settings['background_background'] === 'video' && $settings['background_video_link'] ){
					...
					$element->set_settings('background_video_link', '');
				}

				$classes = [
					'rey-animate-el',
					'rey-anim--' . esc_attr( $settings['rey_animation_type'] ),
					'rey-anim--viewport',
				];
