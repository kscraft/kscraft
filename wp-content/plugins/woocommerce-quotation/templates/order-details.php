...
if ( $order->get_status() != 'proposal-sent' ) { ?>
        <h2><?php _e( 'Quote Details', 'woocommerce-quotation' ); ?></h2>
<?php } ?>
...
<table class="shop_table order_details">
	<thead>
		<tr>
			...
			<th class="product-total"><?php _e( 'Total', 'woocommerce-quotation' ); ?></th>
		</tr>
	</thead>
	<tbody>
		<?php
	...
	<tfoot>
		<?php
		add_filter('woocommerce_order_shipping_to_display', 'adq_shipping_to_display', 10, 2);
		...
		remove_filter('woocommerce_order_shipping_to_display', 'adq_shipping_to_display');
		?>
	</tfoot>
</table>
...
function adq_shipping_to_display($shipping, $order) {
	if ( $order->get_status() == "request" && get_option('adq_hide_shipping_costs') == 'yes' ) {
